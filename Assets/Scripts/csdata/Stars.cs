...
class Stars
{
    ...
    public void ToBinary(StreamWriter sw)
    {
        ...
        sw.WriteLine(scorelast.ToString());
        ...
        sw.WriteLine(isout.ToString());
        sw.WriteLine(lifetime.ToString());
        sw.WriteLine(havetarget.ToString());
        if (havetarget) ship.ToBinary(sw);
        sw.WriteLine(helpcnt.ToString());

        sw.WriteLine(helplist.Count.ToString());
        for (int i = 0; i < helplist.Count; i++)
            sw.WriteLine(helplist[i].ToString());

        sw.WriteLine(techboomcnt.ToString());
        sw.WriteLine(time2d.ToString());
    }
